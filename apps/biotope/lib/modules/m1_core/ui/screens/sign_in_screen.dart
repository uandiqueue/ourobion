...
class SignInScreen extends StatefulWidget {
  ...
  const SignInScreen({super.key, required this.authService, this.onSignedIn});
...
class _SignInScreenState extends State<SignInScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (result.success) {
      // GoTrue saves the password session before `signIn` completes. Notify
      // the gate directly so first sign-in does not depend on the later async
      // auth stream event; OAuth still waits for its redirect event.
      widget.onSignedIn?.call(result.userId!);
