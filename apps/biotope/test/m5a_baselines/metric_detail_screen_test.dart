...
void main() {
  testWidgets(
    'real history renders the shared trend chart and the latest value',
    (tester) async {
  ...
  testWidgets('a metric with NO history says so and paints no chart', (
    ...
    await tester.pumpWidget(
      _host(series: _FakeSeriesService(points: const [])),
    );
    await tester.pumpAndSettle();
    ...
    expect(
      ...
      findsNothing,
      reason:
          'an empty or flat chart would imply a measurement that never '
          'happened',
    );
  ...
  testWidgets('a stale newest reading says how old it is', (tester) async {
    ...
    expect(
      find.textContaining(MetricDetailCopy.recordedDaysAgo(4)),
      findsOneWidget,
    );
  ...
  testWidgets('yesterday is named, not counted', (tester) async {
    ...
    expect(
      find.textContaining(MetricDetailCopy.recordedYesterday),
      findsOneWidget,
    );
  });

  testWidgets(
    'one reading is drawn as one point, with a note instead of a line',
    (tester) async {
      await tester.pumpWidget(
  ...
  testWidgets('named latest readings keep their visual and descriptive label', (
    tester,
  ) async {
    ...
    final cases = [
      (
        ...
        title: 'Urine colour',
        value: 2.0,
        key: const ValueKey('daily-scale-urine_colour-2'),
        visible: '2 - Pale yellow',
        semantic: 'Urine colour 2, Pale yellow',
      ),
      (
        metricKey: 'stool_form',
        title: 'Stool form',
        value: 4.0,
        key: const ValueKey('daily-scale-stool_form-4'),
        visible: 'Type 4 - Smooth sausage',
        semantic: 'Stool form type 4, Smooth sausage',
      ),
    ];
    for (final entry in cases) {
      await tester.pumpWidget(
        _host(
          key: ValueKey('detail-${entry.metricKey}'),
          metricKey: entry.metricKey,
          title: entry.title,
          series: _FakeSeriesService(
            points: [
              MetricDailyPoint(
      ...
      expect(find.text(entry.visible), findsOneWidget);
      expect(find.semantics.byLabel(entry.semantic), findsWidgets);
    }
    handle.dispose();
  });
}
