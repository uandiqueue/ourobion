...
void main() {
  group('InsightsTab swipe-right', () {
    ...
    testWidgets('SAVED count seeds from getArchivedInsights and re-reads on save',
        (tester) async {
      ...
      expect(fake.getArchivedCalls, ['u-1', 'u-1'],
          reason: 'the count after a save must come from the archive query, not '
              'from a local += 1 — an optimistic counter drifts above the true '
              'row count and never re-syncs within the session');
    });

    testWidgets('SAVED count does not drift when the same card is saved twice',
        (tester) async {
      ...
      final fake = _FakeInsightService(
        active: [_card(id: 7), _card(id: 7, title: 'Same row again')],
      );
      await tester.pumpWidget(
        MaterialApp(home: InsightsTab(service: fake, userId: 'u-1')),
      );
      ...
      expect(find.text('1'), findsOneWidget,
          reason: 'two saves of the same row are one archived row; the header '
    ...
    testWidgets('the empty-deck action re-reads Supabase instead of replaying held cards',
        (tester) async {
      ...
      final fake = _FakeInsightService(active: [_card(id: 11, title: 'Only card')]);
      await tester.pumpWidget(
        MaterialApp(home: InsightsTab(service: fake, userId: 'u-1')),
      );
      await tester.pumpAndSettle();
