...
void main() {
  ...
  group('the registry is the source of truth for what is answerable', () {
    ...
    test('the ranges the acceptance criteria name are the ranges in force', () {
      ...
      expect(scales['outside_meals'], (min: 0, max: 10));
    ...
    for (final key in kDailyCoreDqsWeights.keys) {
      testWidgets('$key offers every accepted value, and each one answers', (
        ...
        for (final value in range) {
          if (controlFor(key) == ScanControl.quickCount && value > 3) {
            expect(findQuickOther(key), findsOneWidget);
          } else {
            expect(
        ...
        expect(
          answers,
          equals(range),
          reason: 'every accepted value must write that exact value',
        );
      });
      testWidgets('$key offers nothing outside its accepted range', (
        ...
        expect(
          find.byKey(ValueKey('quick-count-$key-${scale.min - 1}')),
          findsNothing,
        );
        expect(
          find.byKey(ValueKey('quick-count-$key-${scale.max + 1}')),
          findsNothing,
        );
        if (controlFor(key) == ScanControl.quickCount) {
          await tester.tap(findQuickOther(key));
          await tester.pump();
          await tester.enterText(findQuickField(key), '${scale.max + 1}');
          await tester.tap(findQuickApply(key));
          await tester.pump();
          expect(answers, isEmpty);
          expect(
            find.text(
              'Enter a whole number from ${scale.min} to ${scale.max}.',
            ),
            findsWidgets,
          );
        }
