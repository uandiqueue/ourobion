import 'dart:io'; import 'package:flutter/material.dart'; import 'package:flutter_test/flutter_test.dart'; import 'package:src/modules/m2_self_report/impl/logging_controller.dart';
import 'package:src/modules/m2_self_report/impl/normaliser.dart'; import 'package:src/modules/m2_self_report/ui/screens/scan_tab.dart';
import 'package:src/modules/m5a_baselines/index.dart' show metricDisplayLabel; List<String> get dailyCoreKeys => kDailyCoreDqsWeights.keys.toList(); Widget scanHarness(
  Widget child, {
  ...
  final List<String> metricKeys; final Map<String, int?> answered; final Set<String> savingKeys; final void Function(String metricKey, int value)? onAnswer; final String? initiallyOpen;
...
class ScanGapListHostState extends State<ScanGapListHost> {
  String? openGapKey; @override
  void initState() {
    super.initState(); openGapKey = widget.initiallyOpen; }
  @override
    ...
    children: [
      for (final key in widget.metricKeys) ...[
        gapCard(
          ...
          onAnswer: (value) {
            widget.onAnswer?.call(key, value); setState(() => openGapKey = null); },
        ),
        const SizedBox(height: 11),
      ],
    ],
  ); }
String expandedProbe(String metricKey) => ScanTabCopy.inlineHints[metricKey]!; enum ScanControl {
  chips,
  armstrong,
  bristol,
  stepper,
...
ScanControl controlFor(String metricKey) => switch (metricKey) {
  ...
  _ => ScanControl.chips,
    ...
    controlFor(metricKey) != ScanControl.stepper; String optionSemanticLabel(String metricKey, int value) =>
    ...
    }; String stepperReadoutLabel(int value) => 'Mosquito bites, $value selected'; String scanTabSource() => File(
  'lib/modules/m2_self_report/ui/screens/scan_tab.dart',
).readAsStringSync(); String declarationBody(String source, String name) {
  final decl = RegExp(
    r'^(?:abstract final )?(?:class|enum) (\w+)',
    multiLine: true,
  ); final matches = decl.allMatches(source).toList(); for (var i = 0; i < matches.length; i++) {
    if (matches[i].group(1) != name) continue; final end = (i + 1 < matches.length) ? matches[i + 1].start : source.length; return source.substring(matches[i].start, end); }
  throw StateError('declaration "$name" not found in scan_tab.dart'); }
String squashWhitespace(String source) =>
    source.replaceAll(RegExp(r'\s+'), ' '); Finder findGapCard(String metricKey) => find.byWidgetPredicate(
...
Finder get findStepperIncrease =>
    find.widgetWithIcon(IconButton, Icons.add_rounded); Finder get findStepperDecrease =>
    find.widgetWithIcon(IconButton, Icons.remove_rounded); Finder get findStepperSave => find.byKey(const ValueKey('mosquito-save')); int stepperValue(WidgetTester tester) {
...
Future<void> stepStepperTo(WidgetTester tester, int value) async {
  var current = stepperValue(tester); while (current != value) {
    final button = current < value ? findStepperIncrease : findStepperDecrease; await tester.ensureVisible(button); await tester.tap(button); await tester.pump(); final next = stepperValue(tester);
    if (next == current) {
...
Future<void> answerWith(
  ...
  final target = controlFor(metricKey) == ScanControl.stepper
      ? findStepperSave
      : findOption(metricKey, value); if (controlFor(metricKey) == ScanControl.stepper) {
    await stepStepperTo(tester, value); }
  await tester.ensureVisible(target); await tester.tap(target); await tester.pump(); }
