import 'package:flutter/material.dart'; import 'package:flutter_test/flutter_test.dart'; import 'package:src/core/app_preferences.dart'; import 'package:src/modules/m1_core/impl/profile_service.dart';
import 'package:src/modules/m1_core/models/user_profile.dart'; import 'package:src/modules/m1_core/ui/screens/profile_tab.dart'; import 'package:supabase_flutter/supabase_flutter.dart';
import '../../../../shared/constants/copy_guidelines.dart'; class _ProfileTabService extends ProfileService {
  _ProfileTabService(this.profile)
    : super(
      ...
      ); final UserProfile profile; bool digestEnabled = false; final List<Map<String, dynamic>> profileWrites = []; final List<bool> digestWrites = []; @override
  Future<UserProfile?> getProfile(String userId) async => profile; @override
  Future<bool> getDailyDigestEnabled() async => digestEnabled; @override
  ...
  Future<void> setDailyDigestEnabled(bool enabled) async {
    digestWrites.add(enabled); digestEnabled = enabled; }
}
final _profile = UserProfile(
  ...
  group('the actual ProfileTab renders and wires every preference row', () {
    setUp(() => AppPreferences.resetForTest()); tearDown(() => AppPreferences.resetForTest()); testWidgets(
      'all three real rows render their constants and persist through their declared owners',
      (tester) async {
        final service = _ProfileTabService(_profile); await tester.pumpWidget(_profileHarness(service)); await tester.pumpAndSettle(); for (final copy in [
        ...
        await tester.tap(find.text(ProfileTabCopy.wearableLabel)); await tester.pumpAndSettle(); expect(service.profileWrites, [
          {'wearable_owned': true, 'user_id': _profile.userId},
        ]); expect(
          service.digestWrites,
          isEmpty,
          reason: 'wearable ownership is not the digest preference',
        ); expect(AppPreferences.backdropEnabled.value, isTrue); await tester.tap(find.text(ProfileTabCopy.backdropLabel)); await tester.pumpAndSettle(); expect(
        ...
        ); await tester.tap(find.text(ProfileTabCopy.digestLabel)); await tester.pumpAndSettle(); expect(service.digestWrites, [true]); expect(
          service.profileWrites,
          hasLength(1),
          reason:
    ...
    test('the label does not claim an active connection', () {
      ...
      ); expect(ProfileTabCopy.wearableLabel, 'I use a wearable'); }); test(
      'the subtitle does not claim the switch enables or controls syncing',
      () {
        final s = ProfileTabCopy.wearableSubtitle.toLowerCase(); expect(
          s,
        ...
        ); expect(s, isNot(contains('sync'))); expect(s, isNot(contains('controls'))); },
    ...
    test('the subtitle says the preference stays on this device', () {
      ...
      ); expect(ProfileTabCopy.backdropSubtitle.toLowerCase(), contains('only')); }); test('the subtitle describes a visual effect and nothing else', () {
      final s = ProfileTabCopy.backdropSubtitle.toLowerCase(); expect(
        s,
        contains('drifting orbs'),
        reason: 'the described effect is decorative animation, not data',
      ); expect(s, isNot(contains('data'))); expect(s, isNot(contains('sync'))); expect(s, isNot(contains('account'))); expect(s, isNot(contains('log'))); }); });
  group('digest row cannot be read as "notifications are on"', () {
    test('the subtitle never implies a digest is delivered', () {
      final s = ProfileTabCopy.digestSubtitle.toLowerCase(); expect(s, isNot(contains("you'll receive"))); expect(s, isNot(contains('you will receive'))); expect(s, isNot(contains('delivered'))); });
    test('the subtitle states both the real persistence and the real gap', () {
      final s = ProfileTabCopy.digestSubtitle.toLowerCase(); expect(
        s,
      ...
      final s = ProfileTabCopy.digestSaveFailed.toLowerCase(); expect(s, contains('not saved')); expect(s, isNot(contains('success'))); }); });
  group('every preference row is registered and passes the copy gate', () {
    test('all three rows\' strings are in ProfileTabCopy.all', () {
      ...
      expect(ProfileTabCopy.all, isNotEmpty); for (final s in ProfileTabCopy.all) {
        expect(
