...
import { graphContentSha256 } from '../lib/graph_hash.mjs';
...
test('rejects malformed graph shapes', () => {
  assert.throws(() => renderGraphHtml({ nodes: [] }), /graph\.links must be an array/);
  assert.throws(() => renderGraphHtml({ nodes: [{ id: 'same' }, { id: 'same' }], links: [] }), /duplicate id/);
  assert.throws(() => renderGraphHtml({ nodes: [], links: [], hyperedges: {} }), /graph\.hyperedges must be an array/);
  assert.throws(() => renderGraphHtml({ nodes: [], links: [], hyperedges: [{}] }), /graph\.hyperedges\[0\]\.nodes must be an array/);
});
...
test('embeds graph data safely without executable markup', () => {
  ...
  const rendered = renderGraphHtml(hostile, { sourceSha256: 'a'.repeat(64), contentSha256: 'b'.repeat(64) });
  assert.ok(rendered.includes('\\u003c/script\\u003e'));
  assert.ok(!rendered.includes('</script><img'));
});

test('emits an inline runtime script that compiles', () => {
  const rendered = renderGraphHtml(graph);
...
test('uses stable IDs to resolve equal-degree and equal-label ordering ties', () => {
  ...
  const forward = renderGraphHtml(tied, options);
  const reversed = renderGraphHtml({
    ...tied,
