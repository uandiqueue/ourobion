...
/**
 * C10 (docs/shared/phase2-run-config-decisions.md): the provisional cross-metric lag set is
 * {0, 1, 3, 7} days. The blueprint contract encodes lag 0 as `lagDays: null` (schema forbids 0),
 * so the non-null values the engine accepts are {1, 3, 7}. A coincidence rule carrying any other
 * lag is skipped at load and logged — never silently mis-evaluated.
 */
export const ALLOWED_LAG_DAYS: ReadonlySet<number> = new Set([1, 3, 7])
