...
Bootstrap-only exception: PR #156 may merge into `dev-phase2-run4`, never `dev-phase2`, after all exact 14 checks are green at its current head. This exception installs only authority/tracking docs; it does not authorise U0 or any product implementation, which remains gated on ADMIN-capable branch-protection enforcement.
