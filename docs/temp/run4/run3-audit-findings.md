...
| A15 | medium | docs | `docs/temp/run3/*` are `status: canonical` while their own summaries say "not ground truth" | all five run3 docs; `context_sync.mjs:210` | confirmed by reading |
| A16 | medium | docs | ADR 0003 frontmatter says `accepted`, body says "Status: Proposed" — and accepted bodies are frozen | `docs/shared/decisions/0003-paper-reliability.md:6,17` | confirmed by reading |
...
| `context_sync --check` against the post-#145 merge state | Materialising the merge; both `AGENTS.md` and `docs/shared/structure-context.md` changed on the base after the evidence run. |
