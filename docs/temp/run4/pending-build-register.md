...
| B-UI7 · nao production build (next build / OpenNext / CF Worker secrets) | local build proven; hosted delivery still **unverified** — **issue #227** | Fresh #227 evidence: `next build` completed all 19 routes + middleware; installed `opennextjs-cloudflare build` exited 0 and emitted `.open-next/worker.js` (2,278 bytes) in a 180-file / 8,740,393-byte tree. The synthetic `OUROBION_INTERNAL_SECRET` canary was absent from 47 Next client files, 59 OpenNext client files, and the Worker. Trace inspection found no source escape: entries resolved inside `apps/nao` or its validated dependency junction; two apparent outside entries were generated Next runtime dependency copies only (0 non-`node_modules` files). Observed warnings, without a size/warning threshold: OpenNext Windows compatibility, old `compatibility_date`, expected missing local env templates, and an `EPERM` standalone-copy warning caused by the temporary dependency junction. Still untested: actual deployment and Worker-secret delivery. **A green local production build is not deployment evidence** |
