...
| U26 | A10/A11 — budget-ledger lifecycle: concurrency (re-read+merge if small, else document single-writer — build agent's call per D19) + prune old days/runs | **done** | Branch `fix/tools/budget-ledger-lifecycle` / PR #87, issue #86; A10 = re-read+merge SHIPPED in both ledgers (the merge stayed small: element-wise max, which equals the union-sum since every writer persists after each record/charge — run-token cap merges cleanly too, so NO residual single-writer assumption; residual = one-call-wide read→rename race + one-call-stale pre-call gate, both inside the 5% headroom, documented in the llm-router README); A11 = `budget.retentionDays` (optional, default 30) pruning days+runs on load/persist, `state()` bounded, old version-1 ledger files load fine; brain-ingest prunes dead-window counters and its `charge()` gate now fires on MERGED spend. llm-router **48/48** (+6), brain-ingest **323/323** (+3) |
| U27 | A24 — CI blind spots: `deno check` the 3 edge-function handlers + a migration-apply (shadow DB) job | **done** | Branch `ci/deno-check-migrations-apply` / PR #89, issue #88; `deno-check` matrix job (setup-deno v2.x per config.toml's deno_version=2, per-function cwd so each deno.json is the discovered config, `--no-lock`) + `migrations-apply` job (vanilla postgres:17 service = config.toml major_version; ci/migrations-bootstrap.sql roles/auth/extensions + ci/pg-extension-stubs/ pg_cron+pg_net docker-cp'd in; all 16 migrations applied in filename order, ON_ERROR_STOP). Shadow apply proven locally in a disposable postgres:17 container (16/16 clean); `db reset` + `functions serve` still green with the new evaluate-signals/deno.json. CAVEAT: deno absent locally — the `deno check` steps first run for real in CI. Required-checks list grows by 4 (B12 updated) |
| U29 | deno-check CI failure — the U27 job's FIRST real execution (runs 29650507612/29650519111/29650527508) tripped TS2345 in compute-baselines + evaluate-signals: helpers typed `ReturnType<typeof createClient>` (zero-arg overload defaults) aren't assignable from the real `createClient(url, key)` client under supabase-js 2.110's new `PostgrestVersion`/schema generics | **done** | Branch `fix/functions/deno-client-types` / PR #95, issue #94; concrete client type via a `makeClient(url, key)` wrapper (helpers → `ReturnType<typeof makeClient>`, handler instantiates through it — type and instantiation in lockstep) in both failing handlers; import pinned to `jsr:@supabase/supabase-js@2.110.7` (the version CI actually resolved) in ALL THREE handlers — pin over lockfile per the repo's no-deno.lock state; generate-insights needed only the pin. The CI-first-run catch is U27's purpose validated: the job caught a real latent type error on its first execution. `functions serve` + full POST invokes 200 on all three; flutter 66/66 untouched-green |
| U28 | Nit sweep — A8/A9/A21/A22/A23 (A4/A5 done in U19, A7 done in U20): copy-gate word boundaries, stricter Equals guard, stale comments, agree-copy honesty, service-role guard, engine drift nits. A12 is documentation-only (mailbox attestation is moot until real API routes land, SKIP per D15/B5) — add a register/log note, not code | **done** | Branch `fix/sweep/copy-gates-engine-nits` / PR #91, issue #90; A8 = word-boundary + optional-plural matcher (`\b<word>(?:e?s)?\b`) in BOTH copy gates, parity guard now pins the pattern text + \w-only terms, lockstep table vectors both sides; A9 = dead `getCopyRule` stub removed (zero callers); A21 = EDGE_CARD split into with/without-personal variants, "matching pattern" clause ships only when `classified.personal` is a gate-passing row (D15 honesty); A22 = explicit 500 before the Bearer compare in all 3 handlers; A23 = snapshot `window_days` threaded to render, `relationPhrase` throws on non-monotonic, `windowedBaseline` conservative-history doc; A12 note added under B5. rules **64/64** (+6), flutter **66/66** (+4), shared tsc clean, live db reset + all 3 handlers green. shared/constants — **retro-review flag**. **Audit-fix phase complete** (U19–U28 all done) |

**2026-07-18 · audit-fix phase COMPLETE:** U19–U28 all done (U23 subsumed into U22) — every
worklist finding from the A1–A27 audit is now fixed, deliberately skipped by decision (A15/D9),
or documented (A12 → B5 note). What remains is human-gated: PR #72 recovery merge, the stacked
PR chain merge in order, B8 retro-review of the flagged shared/ PRs, B12 required-checks.
...
| 2026-07-18 | U26 budget-ledger lifecycle — retention pruning + concurrent-writer merge (audit A10/A11) | `fix/tools/budget-ledger-lifecycle` (stacked on `fix/db/constraint-hygiene`) / PR #87 · issue #86 | done; A10 re-read+merge shipped in BOTH ledgers (element-wise max = union-sum under persist-per-record; run-token cap merges cleanly → no single-writer assumption left; one-call-wide residual race documented, absorbed by the 5% headroom); A11 `retentionDays` (default 30) prunes days+runs on load/persist, state() bounded, old ledger files load-compatible; brain-ingest gate fires on merged spend + dead-window counters pruned; llm-router 48/48 (+6), brain-ingest 323/323 (+3) |
| 2026-07-18 | U27 CI blind spots — deno-check the 3 edge handlers + shadow-DB migrations apply (audit A24) | `ci/deno-check-migrations-apply` (stacked on `fix/tools/budget-ledger-lifecycle`) / PR #89 · issue #88 | done; two new CI jobs (six total). `deno-check`: setup-deno v2.x matrix over the 3 handler entrypoints, run from each function dir (config discovery stops at its deno.json; evaluate-signals was missing one — added, serve-verified), `--no-lock`. `migrations-apply`: vanilla postgres:17 service + `ci/migrations-bootstrap.sql` (anon/authenticated/service_role, minimal auth.users + auth.uid(), extensions schema) + `ci/pg-extension-stubs/` (pg_cron/pg_net fakes docker-cp'd into the container — `create extension` can't be stubbed in SQL); 16/16 migrations applied clean in a disposable local postgres:17 (proof in session log); `db reset` green after; caveat: no deno locally, the deno-check steps first execute for real in CI; B12 required-checks list +4 |
| 2026-07-18 | U28 final nit sweep — copy-gate word boundaries, agree-copy honesty, service-role guards, engine nits (audit A8/A9/A21/A22/A23 + A12 note) | `fix/sweep/copy-gates-engine-nits` (stacked on `ci/deno-check-migrations-apply`) / PR #91 · issue #90 | done; **shared/ retro-review** (B8, copy-gate matcher change in `shared/constants/`); A8 word-boundary + optional-plural matcher in TS AND Dart (lockstep table vectors both sides; parity guard now pins the pattern text + \w-only terms); A9 dead `getCopyRule` stub removed (no callers); A21 EDGE_CARD honesty split — "matching pattern" clause only with a gate-passing personal signal (`edgeCardTemplate(classified.personal !== null)`); A22 explicit 500 (no secret echo) before the Bearer compare in all 3 handlers; A23 snapshot `window_days` threaded to render + `relationPhrase` throws on non-monotonic + `windowedBaseline` conservative-history doc; A12 attestation note under B5. rules 64/64 (+6), engine-stats 36/36, flutter 66/66 (+4), shared tsc clean, live: db reset + all 3 handlers 200. **Audit-fix phase complete** |
| 2026-07-18 | U29 deno-check unblock — concrete supabase client types + pinned jsr import | `fix/functions/deno-client-types` (plain branch off `dev-phase2` — stack fully merged) / PR #95 · issue #94 | done; TS2345 root cause = `ReturnType<typeof createClient>` picks the zero-arg overload's default generics, not assignable from `createClient(url, key)` under supabase-js 2.110 (`PostgrestVersion` generics); fixed with a `makeClient(url, key)` wrapper typing helpers AND creating the handler client (lockstep by construction); `jsr:@supabase/supabase-js@2` → `@2.110.7` (CI-resolved version, from the failing run logs) in all 3 handlers; validated: serve + full service-role POST → 200 on all three (empty DB), flutter 66/66, context_sync clean; deno-check green proven on the PR's own CI run. U27's job caught a real latent error on its first execution — the finding's point, validated |
| 2026-07-18 | Backend test plan brief (Part-3 deliverable) | `docs/briefs/backend-test-plan` (stacked on `fix/sweep/copy-gates-engine-nits`) | done; `docs/temp/briefs/2026-07-18-backend-test-plan.md` — the repeatable no-app backend verification plan (7 layers: DB / contracts / tool suites / engine functions / brain pipeline / L6 e2e / not-testable register items, + priority order and infra prereqs); all six suite counts re-verified green while writing (323/48/64/36/36/5) and migration count confirmed at 16. **Run complete pending Jayden's merges** (PR #72 recovery, the stacked chain in order, B8 retro-review, B12 required checks) |

| 2026-07-19 | Skills capture (post-run) — orchestration skill set under `.claude/skills/` | `docs/skills/run-procedures` (plain branch off `dev-phase2`) | done; master `orchestrate-build-run` (run loop + dispatch-brief template + tracking-docs reference) + members `stacked-pr-chain` (chain merge procedure, reverse-cascade detection/recovery — the #43–#71 event as cautionary record) + `windows-toolchain-gotchas` (the six recurring traps); every claim verified against session logs / run docs / source before capture |
| 2026-07-19 | Skills generality refactor (post-run) — timeless procedures + example layering | `docs/skills/run-procedures` (PR #97) | done; run-doc naming generalized to `docs/temp/<run-slug>-*.md` + `<integration-branch>` placeholders in the skill set; Phase-2 specifics moved to labeled example blocks (`references/phase2-run-example.md` new); `record-only-audit` + `evidence-review-run` skills committed. NOTE: the convention applies to FUTURE runs — this run's shipped `phase2-run-*.md` docs keep their filenames (renaming shipped run docs is out of scope) |
