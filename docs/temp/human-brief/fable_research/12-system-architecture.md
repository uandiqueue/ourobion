# The complete system architecture — every stage, every wire

**Date:** 2026-07-06 · **Status:** Fable research — end-to-end system-architecture design.
Scope: data-flow, interfaces, stores, and control planes only. Domain terms are opaque labels
(metric = keyed numeric series; paper = document with tier/quote/scope provenance). The
non-diagnostic copy filter (`validateCopyString`) is a fixed black-box invariant, not designed here.
This doc **closes P1–P30** from `11-compilation-issues-and-resolutions.md`; every stage's input is
produced by a named upstream stage or store (no orphan inputs), which is the gaplessness proof the
audit (`10`) demanded. **2026-07-06 revision:** tech-lead decisions applied (P25=A, P26=show,
budget split, hyperparameter registry, PDF=B + citation-block sub-pipeline, S2=view, S1=indefinite,
U1 redefined as LLM transferability) — see §7 for the resolution ledger, §8 for the consolidated
model/compute assignment, §9 for the hyperparameter registry.

**Design invariants inherited (not re-argued):**
- **Two-tier truth** — authoring path may use LLMs; the serving path is deterministic except final
  copy-gated phrasing (`shared/brain/relationships.ts:9-16`).
- **Serving gate** — only `supported|partial` verdicts serve, via `edgeScore`/`servingBand`
  (`shared/brain/index.ts:33,40-57`); 1-hop, monotonic-only (P4).
- **Registry-resolvable endpoints** — every edge endpoint is an active `shared/metrics/registry.ts`
  key (`shared/brain/relationships.ts:18-21,35-36`).
- **TS-first, no Python** (P29): the slice is JATS/structured-XML-first; all gates TS-native.
  **Waiver (decision 5):** the one-language rule is waived for ONE seam — PDF structuring runs a
  GROBID-style structure/reference parser service as a CLI-orchestrated sidecar (A4). Everything
  downstream of the structured artifact stays TS-native.
...
(`metric_daily_values`, the new stage that fixes P14) → **baselines v2** (window-parameterised,
fixing P18) → **3-state signals** → the **n=1 evaluator** (D2, with N_eff + FDR, P5) and the
**composed-insight composer**, which joins a fired signal to 1-hop servable D1 edges and classifies
the branch. The `agree` branch becomes a cited **card** (edge-joined via `edge_refs`, P13); the
`idiosyncratic` branch becomes a **second, clearly-labelled "still researching this" card** —
uncited, visually separate, explicitly an unverified personal observation — AND still fires its
gap-ledger event (P25=A); `research-context` and `contradiction` become **gap-ledger events only**
(P2). Cards are phrased by a cached presentation LLM with a deterministic template fallback, then
selected/clustered/charted into a weekly **report** with an append-only surfaced history (P20).

**Authoring + loop path (LLM-guarded-by-determinism):** the **gap ledger** classifies every candidate
pair (P11/P12) → prioritised **queue** → delivered to the offline ingest CLI over the **existing R2
control plane + GitHub Actions dispatch** (P21, reusing `apps/nao/src/lib/githubDispatch.ts`) →
**pair→query generation** from a new authored **metric↔literature term map** (P15) → retrieval →
**structure-with-offsets** (`extract.ts` v2, P8; GROBID-style parser for PDF-only papers) →
...
(emits claim + quoteSpans + `derivation`, P17) → deterministic **quote check** → **verifier with
corpus-first independent retrieval and a decorrelated non-Anthropic model** (P19) → edge
artifacts in R2 → a nao **loader cron** projects them into Postgres `verified_edges` (no Neo4j, P27)
→ served next report with a novelty boost. **Prepopulation runs this same pipeline off-API** via a
developer-run terminal LLM session (no metered budget); runtime queue runs are metered and governed
by the nao-editable budget in R2 `control/ingest-config.json`.

23 stages designed below. One contract PR (P17) bundles `Citation.population`,
`RelationshipClaim.derivation`, and structured `QuoteSpan` offsets.

---

## 1 · Stage-by-stage map

Compute classes: **DET** deterministic code · **RULES** authored rules/config · **TM** trained model
(deferred; LLM substitute named) · **LLM**. Generalization: **G** one implementation for all metric
keys · **M** needs per-key authored config (location given).
...
| U1 | Applicability transferability grader (redefined) | **LLM** (graded at generation time, cached; serve reads DET) | G | Claude Sonnet 5 (`claude-sonnet-5`); optional decorrelated 2nd grader | `applicability_grades`, `user_attributes` |
...
metered API budget** (see §5b). S8/S9 phrasing runs **server-side at generation time**
(fire-triggered, cached — the IED presentation-agent discipline,
...
The consolidated per-stage model assignment is §8.
...
3. **Compute:** DET. 4. **Generalization:** M — per-key config IS the registry
   ...
   `dqs`).
5. **Transport out:** table read by S2. 6. **Store:** the two tables above; **DECIDED: retained
   indefinitely — no trim policy** (they are TRUTH-tier raw rows). Idempotency: day-level upsert.
7. **Failure:** a missing day is a null in S2; never blocks downstream.

**Load-bearing correction to `08`:** day-level history is **already retained** here — the P14 break
was only that `08` pointed the evaluator at `baseline_snapshots` aggregates. No new capture store is
needed; S2 is the missing read path.

### S2 · Metric joint-series projection — **new** (resolves P14's input)
...
2. **Input:** S1 tables. **Output shape:**
   ...
   ```
   `(user_id, metric_key, day, value numeric|null)`. The view SQL is **generated from the registry**
   (a couplings guard asserts every `baselineApplicable` key appears — same pattern as
   `apps/biotope/test/guards/`, `registry.ts:4-6`).
3. **Compute:** DET (a view; zero copy). 4. **Generalization:** G — one generator for all keys.
5. **Transport:** table read by S3, S5, S7-completeness. 6. **Store:** none — **DECIDED: stays a
   live view** (on-demand freshness, "like a live trend feed"), not a nightly materialized table.
   Materialisation (nightly, upsert key `(user_id, metric_key, day)`) is retained only as a future
   fallback if measured query cost demands it.
7. **Failure:** empty for a user → downstream sees zero rows → S3 emits `insufficient`, S5 emits
   nothing, completeness = 0.

### S3 · Baseline v2 (amends existing — resolves P18)
...
5. **Transport:** nightly cron (existing schedule migration `20260515100001`), writes table; S4/S7
   ...
   **not** retained here — anything needing history reads S2 raw (the P18 "raw-log aggregation"
   choice; cheaper than a snapshot-history table and can never drift from the raw rows).
...
4. **Generalization:** M — per-key deadband lives in a registry extension field
   ...
   (`registry.ts:37-69`; TRUTH-tier, part of the P17 PR bundle or a sibling registry PR).
...
`docs/biotope/INSIGHTS-ENGINE-DESIGN.md:54`) is **renamed `coincidence`** in the blueprint contract;
genuine cross-metric relations are exclusively D1/D2 territory (resolves P28).

### S5 · D2 personal relations — the n=1 evaluator (resolves P14, P5)
...
2. **Input:** S2 view — the two aligned day-series of a pair. Pair set = active-metric pairs where
   ...
   {rho, nDays, nEff, ci, p}` — pure, unit-testable; `signal := q_value ≤ 0.05 ∧ n_eff ≥ 10 ∧ stable`.
...
6. **Store:** above; upsert key `(user_id, metric_a, metric_b)`; `runs_observed`/`stable` give the
   cheap re-evaluation P12 requires. Retention: overwrite-in-place is fine (recomputable from S2).
...
### S6 · D1 edge read store + gating (resolves P27; store for C10)
...
2. **Input:** A11 loader (below). **Output shape (DDL):**
   ...
   create table public.edge_verifications (
     ...
     primary key (edge_id, verified_at)   -- append-only: "edge version" = (edge_id, verified_at)  [P20]
   );
   ...
   Neo4j** (P27): 1-hop lookup = `where subject = $k or object = $k` with two btree indexes; the
   IED line naming a Neo4j projection (`docs/biotope/INSIGHTS-ENGINE-DESIGN.md:22`) is amended.
3. **Compute:** DET. 4. **Generalization:** G. 5. **Transport:** table read by S7, A1; RLS: readable
   by authenticated users (population data, no user rows). 6. **Store:** above; claims upsert on
   `edge_id` (re-synthesis replaces, per `relationships.ts:99-101`); verifications append-only, prior
   rows flipped `status='superseded'` by the loader. 7. **Failure:** zero rows → S7 finds no edges →
   every fired pattern becomes a gap event, no cards — degraded but correct (P9's
...
2. **Input:** S4 `FiredPattern[]` (in-process), S6 `verified_edges` (table read), S5
   ...
   **Output shape:**
   ...
   interface ComposedInsight {
     ...
     edges: Array<{ edgeId: string; verifiedAt: string;      // edge version (P20)
     ...
     completeness: { score: number; daysPresent: number; windowDays: number;
                     perMetric: Record<MetricKey, number> };  // deterministic, P24
   }
   ```
   **Branch rules (RULES, exhaustive):** servable consistent edge ∧ (personal null-or-consistent) →
   ...
   Per P2/P3/P25(=A): `agree` proceeds to S8 as a **cited card** (triangulation is a *rank
   modulator*: agree-with-personal ranks above agree-without). `idiosyncratic` now does BOTH:
   (a) proceeds to S8 as a distinct, clearly-labelled **"still researching this" card** — uncited,
   visually separate, explicitly an unverified personal observation; AND (b) still writes its A1
   gap event (`personal-signal-no-edge`) so the loop goes looking for papers. `research-context`
   and `contradiction` remain **not surfaced** — gap events only (`research-context` →
   ...
   edges attach as context-only citations, never a directional claim (P4).
   **Completeness (P24, P18):** `score = Σ_m w_m · daysPresent(m, window)/windowDays` over the
   pattern's contributing metrics, weights `w_m = dqs.weight`-normalised (`registry.ts:62-63`),
   computed **from S2 raw**, reproducible. Weights declared provisional-pending-calibration.
...
7. **Failure:** no fired patterns → no rows (report says "quiet period" via S9 fallback); no edges →
   ...
   (P25=A); everything else is gap fuel (correct per P2).

### S8 · Card producer + phrasing (resolves P13; two card variants per P25=A)
...
2. **Input:** S7 `agree` rows + S7 `idiosyncratic` rows. **Output shape / store amendment (the P13
   migration):**
   ```sql
   ...
   Upsert ownership: each producer upserts only rows in its own `rule_id` namespace; novelty history
   is NOT this table's job (see `surfaced_cards`, S9) — the in-place refresh stops being a bug (P20).
3. **Compute:** DET assembly + **LLM phrasing** (Claude Haiku 4.5 presentation agent — grounded:
   ...
   the IED §E discipline, `INSIGHTS-ENGINE-DESIGN.md:128-136`). Claim register per P1: causal
   wording only inside quoted-citation framing; personal causal claims blocked by the copy gate
   (opaque here). The still-researching variant's phrasing prompt additionally REQUIRES the
...
4. **Generalization:** G. 5. **Transport:** same nightly job as S7, function call; app reads via
   the existing service (`apps/biotope/lib/modules/m5b_insight_engine/impl/insight_service.dart:93-104`,
   which needs only the three new columns added to `fromJson`).
6. **Store:** `insight_cards` amended; upsert `(user_id, rule_id)`. 7. **Failure:** LLM down/over
   ...
   …"` from blueprint-style templates), card still ships; copy-gate failure → card dropped + logged
   (defense-in-depth, IED §C `render.ts` pattern).

### S9 · Report composer + surfaced history (resolves P20, P22)
1. **Purpose:** dated selection/clustering of cards + charts + completeness disclaimer; the novelty
   memory.
2. **Input:** S8 cards (table read), `surfaced_cards` history (below), S2 series for charts.
   **Output shape:**
   ...
   create table public.surfaced_cards (      -- APPEND-ONLY: the novelty memory (P20)
   ...
   **Selection (DET):** rank = `edgeScore × agree-with-personal boost × novelty`; novelty: pair
   ...
   since last report, or pair newly past a completeness threshold → boost (`08 §5` mechanics, now
   with a real store). **Clustering:** cards sharing an edge endpoint (1-hop only); still-researching
   cards render in their own labelled lane, never interleaved with cited cards. **Narrative:**
   LLM (Claude Sonnet 5) constrained to selected cards' payloads; fallback = ordered card list, no
   paragraph.
...
3. **Compute:** DET + LLM (narrative only). 4. **Generalization:** G. 5. **Transport:** trigger =
   on-open with a 7-day freshness check (generate if stale) — avoids cron fan-out; table read by the
   app. 6. **Store:** above; reports idempotent on `(user_id, period_start)`; `surfaced_cards`
   append-only forever (small). 7. **Failure:** zero cards → report with charts + "still researching
   N pairs" line fed by A12 coverage — silence-as-fuel made visible honestly.

### U1 · Applicability = LLM-graded transferability (REDEFINED; resolves P16, P26)
**Redefinition (decision 8):** the previous U1 — a RULES token/range matcher of user attributes
against `population` strings — was the WRONG definition of applicability. Applicability is now an
**LLM-graded transferability score in [0,1]**: how well a paper's claimed relation (between opaque
entities X, Y) transfers to the served insight's relation (between the user's entities X′, Y′),
when the two relations are related-but-not-identical. Population match is at most one input to that
judgment, not the judgment.
1. **Purpose:** the per-(paper claim, served relation) "how well does this evidence transfer" axis
   — the applicability plot's axis (§4 P16) and its drill-down rationale.
2. **Input:** (a) the paper's claim tuple (subject, relation kind, object, quote spans,
   `Citation.population` — P17 field); (b) the served insight's relation (the S7 pattern pair +
   edge relation); (c) optional user context from `user_attributes` (kept as an auxiliary input,
   table unchanged below). **Output shape:**
   ```sql
   create table public.user_attributes (           -- unchanged; now optional grader context only
   ...
   create table public.applicability_grades (      -- NEW: the stored grade + rationale
...
3. **Compute:** **LLM** — Claude Sonnet 5 (`claude-sonnet-5`) for the grade; **optionally a
   ...
   **fire-triggered at S7 generation time** for new `(edge_id, paper_id, prompt_version)` triples
   ...
   (serving path deterministic) is preserved; U1 joins S8/S9 in the "where LLMs run" list (§1, §8).
4. **Generalization:** G (one rubric prompt for all pairs). 5. **Transport:** grading inside the
   ...
   `user_attributes` upsert `user_id`. 7. **Failure:** LLM down/over budget → `'unknown'` (honest),
   regraded next run; empty grades at cold start → plot renders all-`unknown`.
**Unsolved core (stated honestly; listed in §7 residues):** this grade has **no calibration
baseline and no ground truth** — it cannot be numerically validated today. Containment: (a) the
rubric-anchored grading prompt; (b) the optional decorrelated second grader for agreement; (c) the
rationale is exposed to the user, so the score is *auditable* rather than trusted blind; (d)
`'unknown'` whenever the transfer is too weak to grade. **P26 RESOLVED:** the applicability plot
ships at cold start showing `'unknown'` honestly — no gating; the plot axis is this transferability
grade and its drill-down is the stored rationale.

---

## 3 · Authoring + loop stages (the paper side)

### A1 · Gap ledger + status classifier (resolves P11; contains P12)
1. **Purpose:** per-pair record of *why* each candidate pair isn't served; the loop's demand signal.
2. **Input:** three producers, all now real: (a) registry `derivedFrom[]`
   (`registry.ts:57-58`), (b) A6 co-occurrence index, (c) S5 `personal_signals`; plus S6
   `verified_edges`, S7 gap events, S7 completeness. **Output shape:**
   ```sql
   create table public.gap_ledger (
     ...
     demand integer not null default 0,        -- aggregate fire-count, NO user ids (privacy, 08 §7-5)
     last_ingest_attempt jsonb,                -- {runId, at, outcome: 'edges'|'no-papers'|'no-claims'|'failed'}
     corpus_version text,                      -- watermark for cheap re-eval (P12)
     last_status_change timestamptz not null default now(),
     primary key (metric_a, metric_b, scope)
   );
   ```
   Status rules = `08 §2-3` taxonomy verbatim, now with every input produced: the dangerous-middle
   resolution keys off S7 completeness (low → `blocked-completeness`; high + servable-lit-mismatch →
   applicability note, not a target). `retrieval-exhausted` is new — set by A10's zero-result
   write-back, retried only when `corpus_version` advances (bounded spend, P12).
3. **Compute:** RULES. 4. **Generalization:** G. 5. **Transport:** classifier runs in the weekly S5
   cron (after personal signals refresh) + on A11 loader completion (edge-side changes); read by the
   queue builder (A3). 6. **Store:** above; upsert on PK; `demand` incremented aggregate-only.
7. **Failure:** empty ledger → queue empty → loop idles; classifier is total (every candidate pair
   gets exactly one status).

### A2 · Metric↔literature term map + pair→query generation (resolves P15)
1. **Purpose:** the two-directional bridge between metric keys and literature vocabulary.
2. **Input:** authored data. **Shape (new TRUTH contract, `shared/metrics/terms.ts` — sibling of the
   registry, same 2-reviewer gate, `registry.ts:8-9`):**
   ```ts
...
4. **Generalization:** M (authored per key) with G transforms. 5. **Transport:** imported by the
   ...
   `last_ingest_attempt.outcome='no-terms'` — visible, not silent. Known bounded limitation: entity-
   linking precision/recall floor (`10 §4` core 2) — contained by authored maps + exclude terms.

### A3 · Queue transport + targeted retrieval (resolves P21)
...
2. **Input:** A1 spend-ranked rows (rank `f(personalSignal, litCandidate, demand, servability-gain)`,
   `08 §4-1`). **Transport design (the P21 seam), reusing shipped infrastructure:**
   - **Queue document:** nao (the Cloudflare Worker that already owns the control plane) reads
     `gap_ledger` (Supabase binding), builds
     `control/ingest-queue.json` in the **same R2 bucket** as the corpus — the surface both sides
     already share (`tools/brain-ingest/src/control.ts:2-8,28`):
     ```ts
     interface IngestQueueDoc { queueId: string; createdAt: string;
       items: Array<{ pair: [MetricKey, MetricKey]; priority: number;
                      ...
                      maxPapers: number }>; }       // per-item spend cap (P12)
     ```
   - **Trigger:** nao fires `workflow_dispatch` on `.github/workflows/brain-ingest.yml` with input
     `queue=<queueId>` — exactly the shipped mechanism (`apps/nao/src/lib/githubDispatch.ts:41-79`).
     Cadence: weekly cron in nao + manual UI button. Pause/budget honored via the existing control
     doc (`types.ts:315-321`).
   - **CLI side:** new `--queue <queueId>` mode alongside `--seed` (`seeds.ts:55-64` static topics
     ...
     expansion from any hit (the `08 §4-2` un-cut).
   - **Write-back:** CLI writes `runs/<runId>/outcome.json`
     (`{queueId, perItem: [{pair, outcome, papersFetched, edgeIds}]}`) + edge artifacts (A8/A10) to
     R2. A **nao cron** polls `runs/` (or the workflow calls a nao HTTP callback on completion),
     updates `gap_ledger.last_ingest_attempt`, and invokes A11. Idempotency: `runId` recorded on the
     ledger row; re-delivered outcomes are no-ops.
...
### A4 · Structure: segmentation + offsets + role tagging (resolves P8; the `extract.ts` v2)
1. **Purpose:** turn a fetched paper into span-anchored structured text — sections, sentences,
   char offsets, per-sentence role.
2. **Input:** fetched bytes/XML from A3 (via existing retrieval adapters,
   `types.ts:279-282`). **Output shape (R2 `structured/<paperUid>.json`):**
   ```ts
   ...
   ```
   Replaces the flattening path: `collapseWhitespace` (`tools/brain-ingest/src/extract.ts:33-35`) is
   applied once to build `canonicalText` and never again, so offsets stay stable; `mergePages:true`
   PDF flattening (`extract.ts:51`) and the JATS body walk (`:155-165`) both emit into this shape.
   JATS section kinds come free from `<sec>` tags (the current walker discards them, `:77-88`).
   **PDF-only papers (decision 5 — the one-language rule is WAIVED here):** instead of
   heading-heuristic RULES, PDFs run through a **real structure/reference parser** (a GROBID-style
   service is acceptable), deployed as a CLI-orchestrated sidecar; its TEI-like output is mapped
   into the same `StructuredPaper` shape (sections, sentences, offsets into `canonicalText`), and
   its reference/citation output feeds A4b. JATS stays the preferred path when available.
3. **Compute:** DET (JATS segmentation, sentence split) + sidecar service (PDF structure) +
   **TM (deferred)** for role tagging — cold-start substitute: Claude Haiku 4.5
   (`claude-haiku-4-5`) classifying sentences in batch (cheap, offline, budget-guarded); swap plan:
   distil the LLM's labels into a small TS-runnable classifier (logistic over n-grams or ONNX
   minilm) once ≥5k labelled sentences accumulate; `structureVersion` bump triggers re-structure.
...
### A4b · Citation-block parse: claim→reference mapping — **new** (decision 5)
...
3. **Compute:** DET + RULES — a **citation-format library** does (i) style detection and (ii)
   ...
   offsets (JATS `<xref>` gives it free; numeric/author-year markers are regex-per-detected-style).
   No LLM. 4. **Generalization:** G. 5. **Transport:** in-process, immediately after A4; artifact
   to R2. **Consumers:** A6 (the co-occurrence index gains a `refGraph` leg keyed on resolved
   DOIs/refIds) and A10 (corroboration clustering counts support by independent citation root —
   complements the P6/P19 echo control); A8 receives `claimCites` so synthesis context carries
   which reference backs which sentence. 6. **Store:** R2, idempotent on
...
3. **Compute:** RULES first (workType `review`→5-candidate, registry of design keywords in
   ...
   (haiku) only when rules are inconclusive. 4. **Generalization:** G. 5. **Transport:** in-process.
   6. **Store:** field on `structured/`. 7. **Failure:** unknown → tier 2 (conservative floor) +
   flag for review.
...
2. **Input:** A4 sentences + A2 `METRIC_TERMS` + A4b `CitationBlockIndex` (references). **Output:**
   `mentions` filled on sentences; R2 `index/cooccurrence.json`:
...
3. **Compute:** DET (multi-pattern string match, exclude-term veto). 4. **Generalization:** M via
   the shared term map (no second authoring surface). 5. **Transport:** in-process; index re-emitted
   each run (rebuildable from `structured/`); `version` = corpus watermark consumed by
...
1. **Purpose:** mark each finding sentence asserted / negated / hedged so synthesis can't misread a
   null result as a positive one — and so studied nulls become `no_effect` edges
   (`relationships.ts:45`).
2. **Input:** A4/A6 finding sentences. **Output:** `assertion` field. **Compute:** RULES — TS-native
   cue lexicon (negation: "no significant", "did not", "failed to"; hedges: "may", "suggests") with
   window scoping; the negspacy-replacement P29 requires. 4. **Gen.:** G. 5. **Transport:**
   in-process. 6. **Store:** field on `structured/`. 7. **Failure:** unmatched → `hedged`
   (conservative: hedged sentences are context, never claim evidence).
...
2. **Input:** for a queue item's pair: all `asserted`+`finding` sentences mentioning both keys
   ...
   (`relationships.ts:102-125`) **+ the two P17 contract additions:**
   ```ts
   // shared/brain/relationships.ts — ONE bundled 2-reviewer PR (P17):
   interface Citation { /* existing :76-85 */ population: string | null; }   // per-paper scope (P16)
   ...
   interface RelationshipClaim { /* existing */ derivation: string; }
     ...
     // (never regenerated on view — 09 §3), copy-gated before storage
   ```
   `edgeId = relationKey(...)` (`index.ts:20-22`); `no_effect` claims emitted from negated findings.
3. **Compute:** LLM — Claude Sonnet 5 (`claude-sonnet-5`; generative, highest-stakes),
   ...
   (prepopulation, §5b), JSON-schema-validated output; invalid → retry once → drop. 4. **Generalization:** G (one prompt for all pairs; pair specificity comes from input
   selection). 5. **Transport:** in-process; appends R2 `edges/claims.jsonl`. 6. **Store:** R2
   JSONL — the rebuildable-projection artifact (`relationships.ts:9-12`); idempotent per
...
1. **Purpose:** literal-presence check of every `quoteSpan` before any verifier tokens are spent —
   the verifiability invariant behind the exact-quotes UI (P23).
2. **Input:** A8 claims + A4 `canonicalText` per cited paper. **Output:**
   ...
   `charStart..charEnd` when present, whole-text search fallback. 3. **Compute:** DET. 4. **Gen.:**
   G. 5. **Transport:** in-process, gates A10. 6. **Store:** merged into the verification record.
   7. **Failure:** `allPresent=false` → claim rejected without LLM spend; logged for prompt triage.

### A10 · Verification with independent retrieval (resolves P19)
...
2. **Input:** one A8 claim (verdict target) — **not** the synthesis context. **The retrieval
   mechanism (the P19 design):** independence = *the verifier performs its own search*, not that
   results must differ. Two rungs, both CLI-side:
   - **Corpus-first (always available, free):** query the A6 co-occurrence index for the claim's
     pair → structured papers mentioning both keys → their `finding` sentences. Papers also cited by
     the claim are allowed in retrieval but excluded from `corroboration.supporting` counts (echo
     control, complements P6's clustering). **Corroboration is additionally clustered by
     independent citation root** using the A6 `refGraph` (from A4b): supporting papers that share a
     citation root collapse to one effective source, so `corroboration.supporting` counts
     independent evidence lineages, not copies.
   ...
   **Decorrelation requirement (core design principle):** the verifier MUST be from a DIFFERENT
   vendor family than the synthesizer (A8 = Claude Sonnet 5). A same-family "does this look right?"
   re-ask shares the first model's blind spots and is not verification.
   **Zero-result policy:** retrieval performed + nothing found →
   `verdict='unsupported'` with `independentRetrieval.performed=true` (grounded absence — never
   served, `index.ts:33`), ledger → `retrieval-exhausted`, re-verified only when
   `corpus_version` advances or `promptVersion` bumps (`status='stale'`,
   `relationships.ts:70-73`). Retrieval *not performable* (pause/budget) → `uncertain` + automatic
   re-run next window. This converts the permanent-`uncertain` deadlock into a bounded retry.
3. **Compute:** DET retrieval + LLM verdict. 4. **Gen.:** G. 5. **Transport:** in-process; appends
   R2 `edges/verifications.jsonl`. 6. **Store:** R2 JSONL; append-only (versions =
   `(edgeId, verifiedAt)`). 7. **Failure:** LLM invalid output → retry once → `uncertain`
   (safe default, retried).
...
2. **Input:** R2 `edges/*.jsonl` (A8/A10). **Output:** S6 DDL rows; `edge_score`/`serving_band`
   ...
   (`relationships.schema.ts`, referenced at `relationships.ts:27`); invalid lines quarantined, run
   continues.
3. **Compute:** DET. 4. **Gen.:** G. 5. **Transport:** runs in nao (cron, or invoked by the A3
   write-back callback) using nao's R2 binding + Supabase service key — the CLI never holds DB
   credentials. 6. **Store:** S6 tables; idempotent (claims upsert `edge_id`; verifications insert
   `on conflict do nothing` on `(edge_id, verified_at)`). Full-rebuild supported: truncate + reload
   from R2 (two-tier truth). 7. **Failure:** partial load → loader is per-line transactional;
   re-run completes.

### A12 · Coverage metric
1. **Purpose:** the self-improvement gauge: coverage = served / candidate-derivable, honest under a
   moving denominator.
2. **Input:** A1 ledger (statuses), A6 index version. **Output:**
   ...
   ```
   Coverage deltas are reported **only within the same `denominator_version`** (the `08 §7-2` /
   `10 §4` core-4 containment). 3. **Compute:** DET. 4. **Gen.:** G. 5. **Transport:** computed in
   the A1 classifier run. 6. **Store:** append-only. 7. **Failure:** none (pure aggregate).

---

## 4 · The critical interface resolutions (P13–P21 in one place)

| P | Resolution (schema above) | One-line decision |
|---|---|---|
| **P13** | S8 migration: `producer`, `insight_id` FK, `edge_refs jsonb`, category CHECK + `'relationship'`; edge cards use `rule_id = 'edge:'||edge_id` | One table, two producers, namespaced key spaces; composer (S7→S8) owns edge-card upserts; rules engine untouched |
| **P14** | S2 `metric_daily_values` view over already-retained day rows (`...20260513...sql:49`, `...20260528...sql:16`); S5 evaluator I/O: Spearman ρ, Pyper–Peterman N_eff, BH q-value, 60-day window, 3-run stability | No new capture store — a missing read path, not missing data |
| **P15** | A2 `shared/metrics/terms.ts` (TRUTH, guard-coupled to registry): forward `pairToQueries`, reverse A6 matcher | One authored map powers both directions |
| **P16** | U1 REDEFINED (decision 8): applicability = **LLM-graded transferability** `[0,1]\|'unknown'` + stored grader rationale (Claude Sonnet 5, graded at S7 generation time, cached in `applicability_grades`); `user_attributes` kept as optional grader context; the plot axis = this grade, drill-down = the rationale | 'unknown' rendered honestly (P26 RESOLVED: show, no gating) |
| **P17** | One 2-reviewer PR: `Citation.population`, `RelationshipClaim.derivation`, `QuoteSpan.charStart/charEnd` + Zod mirrors + guards | Bundled; derivation captured at synthesis time only |
| **P18** | S3: `window_days` + `total_history_days` columns; confidence re-based; completeness computed from S2 raw, never from snapshots | Raw-log aggregation chosen over snapshot history |
| **P19** | A10: verifier-owned retrieval = corpus-first (co-occurrence index) + live top-up; corroboration clustered by independent citation root (A4b/A6 `refGraph`); zero-result → `unsupported` + `retrieval-exhausted` with corpus-watermark retry; not-performable → `uncertain` + auto re-run | Independence = verifier's own search + decorrelated non-Anthropic verifier model, echo-controlled corroboration counts |
| **P20** | S9 `surfaced_cards` append-only; edge version := `(edge_id, verified_at)` (matches S6 PK) | Novelty memory decoupled from the card upsert |
| **P21** | A3: Supabase ledger → nao builds R2 `control/ingest-queue.json` → `workflow_dispatch` (`githubDispatch.ts:41-79`) → CLI `--queue` → R2 `runs/<runId>/outcome.json` + edge JSONL → nao cron write-back + A11 load | Reuses the shipped R2 control plane (`control.ts:28`) and dispatch path; CLI stays DB-credential-free |
...
user/device ─► S1 tables ─(view)─► S2 metric_daily_values
  ...
  table read:   U1 applicability_grades (pre-graded, cached)
                     ...
                     S7 composer: join + branch + completeness
                        ...
                        │                   AND A1 gap event (personal-signal-no-edge)   [P25=A]
...
**Budget split (RESOLVED — replaces the old "budget allocation" open item):**
- **Prepopulation (P9/P10) is OFF-API.** The initial edge set is produced by running this exact
  pipeline — same stages, same contracts, same R2 artifacts, same gates — with the LLM stages
  (A4 role-tags, A5, A8, A10) driven **manually via a developer-run terminal LLM session** (e.g.
  Claude Code / an interactive terminal model session) instead of metered API calls. This consumes
  **no metered API budget**. It still EXERCISES the real pipeline: edges are pipeline-produced (A9
  quote gate, A10 decorrelated verification, A11 load all run for real), NOT hand-authored;
  hand-seeded `provenance:'seed'` rows remain anti-empty insurance only. Curated queue seeded from
  `derivedFrom[]` pairs.
- **Runtime (demand-driven ingestion from the gap queue) is METERED.** LLM/API spend is governed
  ...
  R2 control plane: extend `control/ingest-config.json`
  (`tools/brain-ingest/src/control.ts:28`) — `IngestLimits` (`types.ts:302-305`) gains an
  `llmDailyUsd?` override alongside `openalexDailyUsd?`, normalized by `normalizeIngestControl`
  (`control.ts:35-45`), enforced by the CLI's fail-closed `BudgetGuard` (`types.ts:213-223`), and
  surfaced/editable in nao's existing ingestion-controls UI.
...
```
L0  P17 contract PR ──────────────┐        S2 view + generator ─┐
    (population/derivation/offsets)│                            │
L1  A4 structure v2 (needs P17 offsets; GROBID-style PDF sidecar)   S3 baseline fix ── S4 signals
...
L5  S7 composer (needs S4, S5, S6, U1-grader stub returning 'unknown') + S8 card migration (P13,
    incl. 'personal' producer for the still-researching variant)
L6  ONE-CARD END-TO-END SLICE (09 §5): one pair through A2→A11, one agree card,
...
```
Parallelizable tracks (P30): the entire right-hand column (S2–S5, S7 skeleton, migrations) is
deterministic, spec-complete work assignable to a second developer; the left column (A4–A10) is the
LLM-adjacent scored delta. L0 gates L1; nothing else blocks starting both columns immediately.

---

## 7 · Open decisions — resolution ledger (2026-07-06)

1. **P25 — RESOLVED (A: surface it).** `idiosyncratic` is BOTH user-visible as a distinct,
   clearly-labelled "still researching this" card (uncited, visually separate, explicitly an
   unverified personal observation — S8 `producer='personal'`, `edge_refs='[]'` enforced) AND
   still fires the A1 gap-ledger trigger (`personal-signal-no-edge`). S7 branch rules, S8
   migration, and §5(a) control flow updated.
2. **P26 — RESOLVED (show unknown).** The applicability plot ships at cold start showing
   `'unknown'` honestly — no gating. Kept as wired (U1 §7 failure path).
3. **Budget allocation — RESOLVED (prepopulation off-API; runtime metered + nao-visible).**
   Prepopulation runs the real synthesis+verify pipeline manually via a developer-run terminal LLM
   session (e.g. Claude Code) — zero metered API budget, pipeline-produced (not hand-authored)
   edges. Runtime demand-driven ingestion uses metered API calls governed by a budget setting
   modifiable and checkable in the nao app via the extended R2 `control/ingest-config.json`
   (`control.ts:28`, `IngestLimits` + `llmDailyUsd?`). See §5(b).
4. **Calibration constants — RESOLVED (dummy-now + registry).** Every magic constant ships as a
   declared-provisional DUMMY value; §9 "Hyperparameter registry" is the single enumerated list
   (name · location · dummy value · what it controls) so they can be exposed and calibrated later
   without hunting. A user-facing "hyperparameter provenance" surface (value + rationale +
   citation, nao-renderable) is designed as a stub and DEFERRED (§9).
5. **PDF segmentation — RESOLVED (B + citation-block sub-pipeline).** The one-language rule is
   waived: PDF-only papers go through a real structure/reference parser (GROBID-style sidecar) in
   A4, and a new A4b citation-block parse stage (style detect → reference list → per-claim
   claim→reference map) feeds A6's `refGraph` and A10's corroboration-by-independent-root.
6. **S2 materialization — RESOLVED (live view).** `metric_daily_values` stays a live database view
   (on-demand freshness, like a live trend feed). Nightly materialization survives only as a
   future fallback note on measured query cost.
7. **Raw-row retention — RESOLVED (indefinite).** S1 rows are retained indefinitely; no trim
   policy. (If one is ever proposed, S5's 60-day window and completeness windows set the floor.)

**Genuinely open / irreducible residues (not decisions):**
- **U1 transferability calibration (new, decision 8):** the LLM-graded applicability score has no
  calibration baseline or ground truth — numerically unvalidatable today. Contained by: (a)
  rubric-anchored grading prompt, (b) optional decorrelated non-Anthropic second grader for
  agreement, (c) the stored rationale exposed to the user (auditable, not trusted blind), (d)
  `'unknown'` when the transfer is too weak.
- **Hyperparameter values (§9):** all dummies; calibration method and data per constant still open.
- False-signal discrimination (P12), entity-linking error floor (A2/A6), applicability inference
  on scope-less papers, coverage-denominator drift — all bounded by the mechanisms above
  (`retrieval-exhausted` + watermark retry, exclude-terms, `'unknown'`, `denominator_version`),
  none solvable to zero.

---

## 8 · Model & compute assignment (consolidated)
...
same artifacts (§5b). No stage requires the highest tier (Claude Opus 4.8 / Claude Fable 5) today;
escalate A8 only if synthesis quality gates fail.

### 8.2 Trained ("own") models — deferred, with named substitutes
...
## 9 · Hyperparameter registry (all values DUMMY — declared provisional)

Purpose: every magic constant in the system, enumerated once, so each can be exposed and
calibrated later **without hunting it down**. All current values are dummies (decision 4); none
has a derivation, citation, or calibration behind it yet.
...
| Applicability plot bands | plot spec (§4 P16) | 0.33 / 0.66 | Low/mid/high banding of the transferability axis |
...
| A3 per-item spend cap | `IngestQueueDoc.items[].maxPapers` (A3 §2) | per-item, TBD | Bounded spend per queue pair (P12) |
