...
▶ **RESUME AT: F3** — F0 + F1 + F2 done. Assessment complete; worklist below reflects the A1/A2 reframing
...
| 2026-07-19 | F2 C5 medium cutoff revert 5→7 | `fix/research-fixes/c5-medium-cutoff-revert` · PR #<F2-pr> · issue #<F2-issue> | reverted S3 medium confidence cutoff `mediumMinDays` **5→7** (RU5b) at both synced config objects (`compute-baselines/index.ts` — NUL preserved via perl, `wc -c`=1 — + `generate-insights/evaluators.ts`) + comments + dev-aid docs; per-metric backlogged (B1). Behaviour change: 6-day baseline now `low` (was `medium`) — ADDED boundary test; rules **65/65** (+1), engine-stats **36/36**, context_sync + flutter analyze green. Live proof = node boundary test (Deno mirror via CI `deno-check`). **Chain tip = this branch.** |
