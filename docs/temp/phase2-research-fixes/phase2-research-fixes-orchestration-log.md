...
PR pending):** corrected the `|ρ|≥0.3` "medium" mislabel → "conservative ~top-quartile / relatively-large"
...
| F1 | Correct `\|ρ\|≥0.3` "medium" label → "top-quartile / relatively-large" wording (RU4b) | B | **done** | fixed `phase2-run-config-decisions.md:36` + `config.ts` PAIR_GATES doc-comment; wording/comment only, no behaviour change. Branch `fix/research-fixes/rho-effect-size-label`, PR/issue below. **Chain tip = this branch.** |
