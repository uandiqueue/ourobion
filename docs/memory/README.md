...
- [Analysis rules become data, via a two-tier blueprint→table pattern](0007-rules-as-data-two-tier.md) — Insight rules come from git-tracked hand-authored truth or gated paper-extracted brain artifacts and load into a rebuildable Postgres rules table; serving remains deterministic.
- [graphify is the semantic context tool; complementary to the deferred structural graph](0008-graphify-context-tool.md) — graphify indexes the repo into a gitignored machine graph plus one generated tracked human view; project-bounded tooling, incremental semantic refresh, and explicit exclusions keep it useful without replacing curated architecture or couplings.
