...
4. **nao ingestion go-live checklist** (after the main fold). Confirm the nao Worker secrets
   ...
   detail: [`nao/brain-ingestion-design.md`](../implemented/nao/brain-ingestion-design.md),
   [`nao/nao-app-design.md`](../implemented/nao/nao-app-design.md).

## Backlog

5. **Brain support models** — the deferred public-data training (GMI GPU), per
   [`nao/brain-support-models-design.md`](../implemented/nao/brain-support-models-design.md). Blocked on GMI credits;
...
7. **Hackathon** — the Priority-0 eval artifacts (baseline-vs-verifier catch-rate, cost/latency curve,
   one observed refusal) per [`hackathon/hackathon-direction.md`](../hackathon/the_launchpad_challenge/plan/hackathon-direction.md);
