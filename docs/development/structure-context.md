...
│   ├── brain-ingest/          # TS paper-corpus ingestion pipeline (see docs/implemented/nao/brain-ingestion-design.md)
...
│                              #   here — see docs/development/model-training/README.md and human-gates.md.
...
│   │           └── ...            # Other M* modules
│   └── nao/                   # Brain-inspection web app (Next.js/Cloudflare; see docs/implemented/nao/nao-app-design.md)
...
- **YAML front-matter** — every active doc opens with a front-matter block
  ...
  it. Memory facts (`docs/memory/*.md`) and decisions (`docs/development/decisions/*.md`) use the id-carrying
...
- **excluded** — `docs/sessions/` (append-only logs), `docs/archive/**` (kept verbatim behind an archive
  banner), `assets/ui-generation/biomech-botanical/` (asset-generation working files — manifest/prompts/reviews, not
