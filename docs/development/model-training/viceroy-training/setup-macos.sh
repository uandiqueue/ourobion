#!/usr/bin/env bash
# Viceroy Causal-Language-Risk v0 — one-time environment setup for Apple Silicon.
#
...
if [ -z "$PY" ]; then
  ...
  echo "   or torch 2.4.1 — this is a hard blocker, not a warning."
  echo
...
echo "installing pinned dependencies (~130 MB download, ~550 MB installed)..."
...
for name, mod in [("torch", torch), ("transformers", transformers), ("numpy", numpy)]:
...
if mps_ok:
    ...
    print("MPS unavailable -> training will run on CPU. Still fine for this corpus, just slower.")
PYCHECK

echo
echo "== setup complete =="
echo "Next:  source .venv/bin/activate  &&  python -m viceroy.cli preflight"
