...
- Added `dev-phase2-run4` to CI push and pull-request branch filters. Bootstrap PR #156 scope now includes only this minimal CI enablement plus authority/tracking docs; parent head `792f8ad` has 14 green, while the final delta requires local/context/diff validation and is not tested by that prior run.
...
- Recorded the narrow bootstrap-only exception: PR #156 may merge to `dev-phase2-run4` when parent `792f8ad` has 14 green and final CI-filter/tracking-only delta passes local/context/diff validation. The prior run does not test final delta.
